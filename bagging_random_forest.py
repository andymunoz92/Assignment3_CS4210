...
if __name__ == '__main__':
    ...
    for k in range(
        ...
        for i, testSample in enumerate(dbTest):
            ...
            if k == 0:  # for only the first base classifier, compare the prediction with the true label of the test sample here to start calculating its accuracy
                y_test.append(int(testSample[-1]))
    ...
    for vote in classVotes:
        ...
        for i in range(len(vote)):
            if vote[i] > maxVotes:
                maxVotes = vote[i]
                majorityVote = i
        y_ensemble.append(majorityVote)
    accuracy = metrics.accuracy_score(y_test, y_ensemble)
    ...
    for instance in dbTest:
        rf_prediction = int(clf.predict([instance[:-1]])[0])
        y_randomForest.append(rf_prediction)
    # compare the Random Forest prediction for each test sample with the ground truth label to calculate its accuracy
    # --> add your Python code here
    accuracy = metrics.accuracy_score(y_test, y_randomForest)
